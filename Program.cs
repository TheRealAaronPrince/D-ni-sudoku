...
namespace d_ni_sudoku
{
	class Program
	{
		...
		static int[,] grid = new int[(size*size), (size*size)];
		static string s;
		...
		public static void ChangeTwoCell(ref int[,] grid, int findValue1, int findValue2)
		{
			...
			for (int i = 0; i < (size*size); i += 5)
			{
				for (int k = 0; k < (size*size); k += 5)
				{
					for (int j = 0; j < 5; j++)
					{
						for (int z = 0; z < 5; z++)
		...
		public static void Update(ref int[,] grid, int shuffleLevel)
		{
			for (int repeat = 0; repeat < shuffleLevel; repeat++)
			{
				...
				ChangeTwoCell(ref grid, rand.Next(1, (size*size)), rand2.Next(1, (size*size)));
			}
		}
		public static void Main(string[] args)
		{
			...
			for(int bmpX = 0; bmpX < (size*size); bmpX++)
			{
				for(int bmpY = 0; bmpY < (size*size); bmpY++)
				{
					Bitmap number = (Bitmap)Image.FromFile("graphics/" + puzzle[bmpX*(size*size)+bmpY] +".png");
			...
			for(int bmpX = 0; bmpX < (size*size); bmpX++)
			{
				for(int bmpY = 0; bmpY < (size*size); bmpY++)
				{
					Bitmap number = (Bitmap)Image.FromFile("graphics/" + ç1kt1[bmpX*(size*size)+bmpY] +".png");
					graphics.DrawImage(number,bmpX*18 + 24,bmpY*18 + 6);
				}
			}
